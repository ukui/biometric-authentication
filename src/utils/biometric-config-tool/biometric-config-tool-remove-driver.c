...
GOptionEntry remove_driver_entries[] = {
...
gboolean
biometric_config_remove_driver (int argc, char **argv, GError **error)
{
	int i = 0;

	GOptionContext *context;
	context = g_option_context_new (_("Driver1 [Driver2 ...]"));
	g_option_context_add_main_entries (context, remove_driver_entries, NULL);
	...
	for (i = 1; i < argc; i++)
	{
		g_key_file_remove_group (bio_config_file,
								 argv[i],
								 NULL);
	}
