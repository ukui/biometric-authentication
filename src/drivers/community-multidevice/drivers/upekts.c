...
static const struct usb_id upekts_id_table[] = {
	{ .idVendor = 0x0483, .idProduct = 0x2016, .description = "" },
	{ 0, 0, NULL },
};
...
int ops_configure(bio_dev * dev, GKeyFile * conf)
{
	...
	dev->device_name = UPEKTS_NAME;
	dev->full_name = "UPEK TouchStrip";
	...
	dev->usb_info.id_table = upekts_id_table;
	...
	priv->community_driver_id = UPEKTS_NAME;
