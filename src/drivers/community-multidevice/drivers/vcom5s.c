...
static const struct usb_id vcom5s_id_table[] = {
	{ .idVendor = 0x061a, .idProduct = 0x0110, .description = "Veridicom 5thSense" },
	{ 0, 0, NULL },
};
...
int ops_configure(bio_dev * dev, GKeyFile * conf)
{
	...
	dev->device_name = VCOM5S_NAME;
	dev->full_name = "Veridicom 5thSense";
	...
	dev->usb_info.id_table = vcom5s_id_table;
	...
	priv->community_driver_id = VCOM5S_NAME;
